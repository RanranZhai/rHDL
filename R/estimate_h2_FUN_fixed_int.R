...
llfun_fixed_int <- function(param, N, M, Nref = 1000, lam, bstar, lim = exp(-18)){
  h2 = param
  int = 1
...
estimate_h2_fixed_int <- function(Z, N = 1e4, eigen = .9999) {
  Nref <- N
  M <- length(Z)
  ...
  opt <- optim(h11.wls[2], llfun_fixed_int, N = N, Nref = Nref, lam = lam, bstar = bstar1, M = M,
               lim=exp(-18), method ='L-BFGS-B', lower=c(0), upper=c(Inf), hessian = T)
