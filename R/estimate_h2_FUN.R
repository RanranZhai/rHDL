...
estimate_h2 <- function(Z, N = 1e4, eigen = .9999, int_cons = c(0,Inf), tau_cons = c(0,Inf)) {
  Nref <- N
  M <- length(Z)
  ...
  opt <- optim(c(h11.wls[2],1), llfun, N = N, Nref = Nref, lam = lam, bstar = bstar1, M = M,
               lim=exp(-18), method ='L-BFGS-B', lower=c(tau_cons[1],int_cons[1]), upper=c(tau_cons[2],int_cons[2]), hessian = T)
